...
from autoencoder_model import REDNet10, REDNet20, REDNet30
import torch
    ...
    with open(metrics_file, mode="w", newline="") as f:
        ...
        while True:
            ...
            for low_res, high_res in train_loader:
                ...
                if masked_loss:
                    mask = (high_res > -1)  # Only calculate loss where data is valid
                    loss = criterion(outputs[mask], high_res[mask])
                    # logger.info("Outputs")
                    # logger.info(outputs[mask])
                    # logger.info("HR")
                    # logger.info(high_res[mask])
                    # sys.exit()
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=int, required=True, choices=[10, 20, 30], help="Choose model: 10, 20, or 30.")
