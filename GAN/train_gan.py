...
# Custom imports
from gan_model import REDNetGenerator, SRGANDiscriminator
...
def train_gan(args):
    ...
    file_name = f"REDNetGAN_batch{args.batch_size}_samples{args.nr_samples}.pth"
    ...
    # === Models ===
    generator = REDNetGenerator(in_channels=2, out_channels=2).to(device)
    ...
    logger.info("Starting REDNetGAN training...")
    ...
    with open(metrics_file, mode="w", newline="") as f:
        ...
        while True:
            ...
            for low_res, high_res in train_loader:
                ...
                fake = generator(low_res).detach()
                # adding noise to stop discriminator from saturating
                # high_res += 0.05 * torch.randn_like(high_res)
                # fake += 0.05 * torch.randn_like(fake)
                ...
                gen_output = generator(low_res)
                pred_fake = discriminator(gen_output)
                loss_G_recon = criterion_recon(gen_output, high_res)
                loss_G_adv = criterion_adv(pred_fake, valid)
                adv_weight = min(0.1, epoch * 0.01)
                loss_G = loss_G_recon + adv_weight * loss_G_adv
            ...
            with torch.no_grad():
                for low_res, high_res in val_loader:
                    ...
                    loss = criterion_recon(output, high_res)
                    val_loss += loss.item()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train REDNetGAN")
