...
def main(args):
    ...
    file_name = f"ELAN_light{args.model}_24h_batchsize{batch_size}_samples{nr_samples}_normalized{add_masked_loss}_save_attention.pth"
    ...
    if model_class is None:
        raise ValueError(f"Invalid model number {args.model}. Choose from 1.")
    ...
    logger.info(f"Training ELAN_light{args.model} for a max of 24 hours with early stopping")
    ...
    with open(metrics_file, mode="w", newline="") as f:
        ...
        while True:
            ...
            for low_res, high_res in train_loader:
                ...
                # low_res, high_res = low_res[:, :, ::2, ::2], high_res[:, :, ::2, ::2] # scale down
            ...
            logger.info(f"{epoch}, {max_memory}, {elapsed_time}, {train_loss_avg}, {val_loss_avg}")
            ...
            if time.time() - start_time > max_duration:
                logger.info(f"Time limit reached. Stopping training.")
                break

    logger.info(f"Training completed after {epoch} epochs. Best model saved at {model_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script to train ELAN Transformer.")
    parser.add_argument("--model", type=int, required=True, choices=[1], help="Choose model: .")
